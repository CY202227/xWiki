...
class WikiViewer:
    ...
    def display_all_entities(self):
        ...
        # 获取所有实体
        entities = conn.execute(f"SELECT * FROM {self.wiki_table} ORDER BY last_updated_at DESC").fetchall()
        ...
        for idx, entity in enumerate(entities):
            ...
            updated = entity['last_updated_at']

            # 获取来源文件的标题
            source_titles = []
            if sources:
                placeholders = ",".join(["?"] * len(sources))
                rows = conn.execute(f"SELECT title FROM {self.meta_table} WHERE article_id IN ({placeholders})", sources).fetchall()
                source_titles = [row['title'] for row in rows]

            print(f"[{idx+1}] 实体名称: {name} ({domain})")
            print(f"    更新时间: {updated}")
            print(f"    最新共识: {consensus[:200]}..." if len(consensus) > 200 else f"    最新共识: {consensus}")
