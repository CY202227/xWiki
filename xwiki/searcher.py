...
class XWikiSearcher:
  ...
  def _fts(self, conn: sqlite3.Connection, query: str, limit: int) -> List[Dict[str, Any]]:
    rows = conn.execute(
  ...
  def _like(self, conn: sqlite3.Connection, tokens: list[str], limit: int) -> List[Dict[str, Any]]:
    if not tokens:
      return []
    terms = []
    params: List[str] = []
